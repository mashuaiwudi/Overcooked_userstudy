...
class Overcooked_MA_equilibrium_openspace_flexible(Overcooked_equilibrium):
        ...
        for idx, agent in enumerate(self.agent):
            ...
            if self.macroAgent[idx].cur_macro_action_done:
                ...
                if self.macroActionName[macro_actions[idx]] == "get plate 1" or self.macroActionName[macro_actions[idx]] == "get plate 2":
                    ...
                    toplate1 = self.shortest_path_through_zeros(agent.pomap, agent.x, agent.y, self.plate[0].x, self.plate[0].y)
                    toplate2 = self.shortest_path_through_zeros(agent.pomap, agent.x, agent.y, self.plate[1].x, self.plate[1].y)

                    if toplate1 == -1 and toplate2 != -1:
                        which_lettuce = 4

                    if toplate2 == -1 and toplate1 != -1:
                        which_lettuce = 3
                    ...
                    if toplate1 != -1 and toplate2 != -1 and toplate1 <= toplate2:
                        which_lettuce = 3
                    if toplate1 != -1 and toplate2 != -1 and toplate1 > toplate2:
                        which_lettuce = 4
            ...
            # 如果mac action是chop
                ...
                rows = len(agent.pomap)
                cols = len(agent.pomap[0])

                # 遍历所有空 counter
                for x_i in range(rows):
                    for y_i in range(cols):

                        # 如果放有盘子或蔬菜，该位置通常不会再显示为 counter
                        if ITEMNAME[agent.pomap[x_i][y_i]] != "counter":
                            continue

                        # 当前 agent 到该 counter 的最短路径
                        dist_agent1 = self.shortest_path_through_zeros(
                            agent.pomap,
                            agent.x,
                            agent.y,
                            x_i,
                        ...
                        # 队友到该 counter 的最短路径
                        dist_agent2 = self.shortest_path_through_zeros(
                            teammate.pomap,
                            teammate.x,
                            teammate.y,
                            x_i,
                        ...
                        # 只有两个 agent 都能到达，才作为交接 counter
                        if dist_agent1 == -1 or dist_agent2 == -1:
                            continue

                        counter_x.append(x_i)
                        counter_y.append(y_i)

                        distance_to_agent1.append(dist_agent1)
                        distance_to_agent2.append(dist_agent2)
                ...
                for i in range(len(counter_x)):
                    ...
                    dist1 = distance_to_agent1[i]
                    dist2 = distance_to_agent2[i]
                ...
                if best_index is None:
                    ...
                    # 没拿物品时，只需要走到 counter 附近
                    if (
                        self._calDistance(
                            agent.x,
                            agent.y,
                            self.target_counter_x,
                            self.target_counter_y
                        ) == 1
                        and not agent.holding
                    ):
                        primitive_action = ACTIONIDX["stay"]
                        self.macroAgent[idx].cur_macro_action_done = True
                        ...
                        # 保留原来的导航/放置行为
                        primitive_action = self._navigate(
                            agent,
                            self.target_counter_x,
                            self.target_counter_y
                        )
