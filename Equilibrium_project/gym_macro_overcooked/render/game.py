...
import numpy as np
from .utils import *
from ..items import Tomato, Lettuce, Plate, Knife, Delivery, Agent, Food, DirtyPlate, BadLettuce
...
class Game:
    def __init__(self, env):
        ...
        """马帅添加"""
        self.intentions = []  # List to store intention positions
    ...
    # """马帅添加"""
    # def highlight_intention(self, x, y):
    #     """Highlight the grid border to indicate robot's intention."""

        
    #     self.intentions.append((x, y))
    #     print(self.intentions)

    # def clear_intentions(self):
    #     """Clear the highlighted intentions."""
    #     self.intentions = []



    def on_render(self, intentions, abi_values=None):
        ...
        for x in range(self.env.xlen):
            for y in range(self.env.ylen):
                ...
                if self.env.map[x][y] == ITEMIDX["counter"]:
                    ...
                    for k in self.env.delivery:
                        if k.x == x and k.y == y:
                            if k.holding:
                                ...
                                if k.holding.name == "plate" or k.holding.name == "dirtyplate":
                ...
                elif self.env.map[x][y] == ITEMIDX["badlettuce"]:
                    fill = pygame.Rect(sl[0], sl[1], self.scale, self.scale)
                    pygame.draw.rect(self.screen, Color.COUNTER, fill)
                    pygame.draw.rect(self.screen, Color.COUNTER_BORDER, fill, 1)
                    for l in self.env.badlettuce:
                        # print(self.env.badlettuce)
                        if l.x == x and l.y == y:
                            # print(l.name)
                            self.draw(l.name, self.tile_size, sl)
                    ...
                    for p in self.env.plate:
                        if p.x == x and p.y == y:
                            if p.containing:
                                # print(p.containedName)
                                self.draw(p.containedName, self.container_size, self.container_location((y, x)))
                elif self.env.map[x][y] == ITEMIDX["dirtyplate"]:
                    fill = pygame.Rect(sl[0], sl[1], self.scale, self.scale)
                    pygame.draw.rect(self.screen, Color.COUNTER, fill)
                    pygame.draw.rect(self.screen, Color.COUNTER_BORDER, fill, 1)
                    self.draw('dirtyplate', self.tile_size, sl)
                    for p in self.env.dirtyplate:
                        if p.x == x and p.y == y:
                            if p.containing:
                                self.draw(p.containedName, self.container_size, self.container_location((y, x)))
                    ...
                    for agent in self.env.agent:
                        if agent.x == x and agent.y == y:
                            ...
                            if agent.holding:
                                ...
                                elif isinstance(agent.holding, DirtyPlate):
                                    self.draw('dirtyplate', self.holding_size, self.holding_location((y, x)))
                                    if agent.holding.containing:
                                        self.draw(agent.holding.containedName, self.holding_container_size, self.holding_container_location((y, x)))
                                else:
                ...
                """马帅添加"""
                # # Highlight if this tile is in intentions
                # if (x, y) in intentions:
                #     fill = pygame.Rect(sl[0], sl[1], self.scale, self.scale)
                #     pygame.draw.rect(self.screen, Color.HIGHLIGHT_BORDER, fill, 10)
        ...
        # 最后加在屏幕上方显示 predicted ABI
        if abi_values is not None:
            ability, benevolence, integrity = abi_values
            font = pygame.font.SysFont("Arial", 24)
            text_surface = font.render(
                f"Predicted Ability: {ability:.2f}  Benevolence: {benevolence:.2f}  Integrity: {integrity:.2f}",
                True,
                (255, 255, 255)
            )
