...
POLICY_POOL_PATH = r"../final_policy_pool_openspace_coplay"
...
def load_policy_pool(pool_dir):
    """
    从 policy pool 目录下加载所有 agent0 policy。
    每个子文件夹下的 model_5000000.zip 视为一个 policy。
    """
    ...
    for folder_name in sorted(os.listdir(pool_dir)):
        ...
        model_path = os.path.join(folder_path, "model_5000000.zip")
    ...
    if len(models) == 0:
        raise RuntimeError(
            f"在 {pool_dir} 下没有成功加载任何 model_5000000.zip"
        )
...
def train_one_combo(step_penalty_agent1: int, 
    ...
    save_dir_agent1 = os.path.join(
        ...
        f"[coplay][flexible][openspace]agent1_{combo_tag}"
    )
