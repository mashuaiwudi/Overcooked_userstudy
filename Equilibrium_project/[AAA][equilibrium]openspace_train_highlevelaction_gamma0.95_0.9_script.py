...
def check_action_benevolence(env, action_up, action_down, firsttime_down_go_to_counter, firsttime_up_get_counter_lettuce):
    ...
    counter1 = ITEMNAME[env.map[0][1]]
    counter2 = ITEMNAME[env.map[0][2]]
    counter3 = ITEMNAME[env.map[0][3]]
    counter4 = ITEMNAME[env.map[0][5]]
    counter5 = ITEMNAME[env.map[0][6]]
    counter6 = ITEMNAME[env.map[1][0]]
    counter7 = ITEMNAME[env.map[1][7]]
    counter8 = ITEMNAME[env.map[3][7]]
    counter9 = ITEMNAME[env.map[4][1]]
    counter10 = ITEMNAME[env.map[4][2]]
    counter11 = ITEMNAME[env.map[4][5]]
    counter12 = ITEMNAME[env.map[4][6]]
...
def train_one_combo(step_penalty_agent0: int, step_penalty_agent1: int, helping0: bool, helping1: bool):
    ...
    mac_env_id = "Overcooked-MA-equilibrium-v6"
    env_params = {
        "grid_dim": [5, 8],
        "task": ["lettuce salad"],
    ...
    combo_tag = f"a0sp_{step_penalty_agent0}_a1sp_{step_penalty_agent1}_helping0_{helping0}_helping1_{helping1}_gamma0.95_0.9"

    log_dir = os.path.join("logs", combo_tag)
    os.makedirs(log_dir, exist_ok=True)
    new_logger = configure(log_dir, ["csv", "tensorboard"])  # 不输出 stdout

    save_dir_agent0 = os.path.join("final_trained_models", f"[equilibrium][openspace]agent0_{combo_tag}")
    save_dir_agent1 = os.path.join("final_trained_models", f"[equilibrium][openspace]agent1_{combo_tag}")
